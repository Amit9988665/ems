...
public class TaskServiceTest<HttpServletRequest> {
	...
	TaskStatusDTO taskStatusDTO = new TaskStatusDTO();
		...
		taskStatusDTOs1 = taskServiceImpl.getAllTask(null,null);
	 ...
	 * 
	 * TaskDTO taskDTO = new TaskDTO(); taskDTO.setAssignTo("Ankit");
	 * taskDTO.setDescription("this is task for test"); taskDTO.setEffortHours("7");
	 * taskDTO.setEmployeeId(1); taskDTO.setEndDate("09-11-2022T14:44");
	 * taskDTO.setEstimatedHours("12"); taskDTO.setId(1);
	 * taskDTO.setStartDate("08-11-2022T14:44");
	 * taskDTO.setTaskName("my first task");
	 * taskDTO.setUpdateTaskDate("08-11-2022 21:09:56");
	 * taskDTO.setCreateTaskDate("08-11-2022 21:09:56"); taskDTO.setTaskStatusId(1);
	 * 
	 * Optional<Task> task2 = Optional.of(new Task()); Task task = task2.get();
	 * task.setTaskName(taskDTO.getTaskName());
	 * task.setDescription(taskDTO.getDescription());
	 * task.setStartDate(taskDTO.getStartDate());
	 * task.setEndDate(taskDTO.getEndDate());
	 ...
	 * task.setEmployeeId(taskDTO.getEmployeeId());
	 * task.setAssignTo(taskDTO.getAssignTo());
	 * task.setCreateTaskDate(taskDTO.getCreateTaskDate()); task.setTaskStatusId(1);
	 * 
	 * Task updateTasks = null; when(taskRepository.save(task)).thenReturn(task);
	 * when(taskRepository.findById(taskDTO.getId())).thenReturn(task2); try {
	 * 
	 * updateTasks = taskServiceImpl.updateTask(taskDTO); } catch (Exception e) {
	 * 
	 * } assertThat(updateTasks).isEqualTo(task); }
	 */
