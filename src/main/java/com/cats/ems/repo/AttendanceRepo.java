...
public interface AttendanceRepo extends JpaRepository<Attendance, Long> {
	...
	@Query(value=" select * from attendance a where a.employee_id = :id and TO_TIMESTAMP(a.sign_in_time, 'dd-MM-yyyy HH24:MI:ss') >= \r\n"
			...
			+ " TO_TIMESTAMP(a.sign_in_time, 'dd-MM-yyyy HH24:MI:ss') <= TO_TIMESTAMP( :sign_out_time, 'dd-MM-yyyy HH24:MI:ss' ) ORDER By id ASC", nativeQuery = true )
	...
	@Query(value = "	select * from attendance a where a.employee_id = :id and TO_TIMESTAMP(a.sign_in_time , 'dd-MM-yyyy HH24:MI:ss') >= \r\n"
			+ "			  TO_TIMESTAMP(:sign_in_time, 'dd-MM-yyyy HH24:MI:ss') and \r\n"
			+ "			  TO_TIMESTAMP(a.sign_in_time  , 'dd-MM-yyyy HH24:MI:ss') <= TO_TIMESTAMP( :end_date, 'dd-MM-yyyy HH24:MI:ss')  and  a.sign_out_time  IS null ", nativeQuery = true)
	public List<Attendance> findByEmployeeIdAndSignOutTimeIsNull(@Param("id") long employeeid,
            @Param("sign_in_time") String formatDateTime, @Param("end_date") String endDate);
