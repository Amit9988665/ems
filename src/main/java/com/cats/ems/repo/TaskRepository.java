...
//	@Query(value="Select * from task t where t.employee_id in (select e.employee_id from employee e where e. manager_id=2) and start_date='18-11-2022 15:20:00' order by update_task_date desc\r\n"
//			+ "",nativeQuery = true)
//	List<Task>  findByManagerIdAndDate(@Param("id") long token, @Param("start_date") String startDate);

	@Query(value="select * from task t where t.employee_id in (select e.employee_id from employee e where e. manager_id=:id) and TO_TIMESTAMP(t.start_date, 'dd-MM-yyyy HH24:MI:ss') >=\r\n"
			+ "			  TO_TIMESTAMP(:start_date, 'dd-MM-yyyy HH24:MI:ss') and \r\n"
			+ "			  TO_TIMESTAMP(t.start_date, 'dd-MM-yyyy HH24:MI:ss') <= TO_TIMESTAMP(:end_date , 'dd-MM-yyyy HH24:MI:ss')\r\n"
			+ "			" , nativeQuery = true )
	List<Task> findByManagerIdAndDate(@Param("id") long token, @Param("start_date") String startDate, @Param("end_date") String endDate);
