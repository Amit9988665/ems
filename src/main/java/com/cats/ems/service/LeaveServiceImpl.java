...
public class LeaveServiceImpl implements LeaveService {
	...
	public Leave applyLeave(HttpServletRequest httpServletRequest, LeaveApplyDTO leaveApplyDTO) throws ParseException {
		...
		List<Holiday> holidays = holidayRepository.findByStartAndEndDate(stDate, enDate);
		int holydayCount = holidays.size();
		...
		holidays.forEach(e -> {
			String stDate1 = e.getStart() + " " + "00:00:00";
			LocalDateTime dateTime1 = LocalDateTime.parse(stDate1, myFormat);
			DayOfWeek expected1 = dateTime1.getDayOfWeek();
			DayOfWeek day1 = DayOfWeek.SATURDAY;
			DayOfWeek day2 = DayOfWeek.SUNDAY;
			if (expected1 == day1 || expected1 == day2) {
				dayHolyDaySunday--;
			}
		});
		...
		long leftDaysWithoutHolydays = leftDay + 1;
		long leftDaysWithoutHolyday = leftDaysWithoutHolydays - holydayCount;
		leftDays = leftDaysWithoutHolyday - dayHolyDaySunday;
		dayHolyDaySunday = 0;
	...
	public Leave updateLeaveStatus(HttpServletRequest httpServletRequest, long leaveId, long statusId)
		...
		if (statusId == 3) {
			...
			long leftDaysWithoutHolydays = leftDay + 1;
			long leftDaysWithoutHolyday = leftDaysWithoutHolydays - holydayCount;
			long leftDays = leftDaysWithoutHolyday - dayHolyDaySunday;
			dayHolyDaySunday = 0;
	...
	public List<LeaveAllEmployeeRoot> getAllEmployeeByManagerId(HttpServletRequest httpServletRequest,
		...
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm:ss");
		List<Leave> leaves = leaveApplyRepository.findByManagerId(jwtTokenUtil.getToken(httpServletRequest), dateTime,
				dateTime1);
		List<Employee> employees = employeeRepository
				.findEmployeeByManagerId(jwtTokenUtil.getToken(httpServletRequest));
		leaves.forEach(e -> {
			if (e.getLeaveStatusId() != 0) {
				...
				leaveAllEmployeeRoot.setName(employee.getName());

				String startDate = e.getStartDate() + " " + "00:00:00";
				String lastDate = e.getEndDate() + " " + "00:00:00";

				LocalDateTime startDateTime1 = LocalDateTime.parse(startDate, formatter);
				LocalDateTime endDateTime = LocalDateTime.parse(lastDate, formatter);

				while (startDateTime1.compareTo(endDateTime) <= 0) {

					LeaveAllEmployee allEmployee = new LeaveAllEmployee();
					LocalDateTime nextDate = startDateTime1.plusDays(1);
					DayOfWeek expected1 = startDateTime1.getDayOfWeek();
					DayOfWeek day1 = DayOfWeek.SATURDAY;
					DayOfWeek day2 = DayOfWeek.SUNDAY;
					if (expected1 == day1 || expected1 == day2) {
						...
						String setDate = startDateTime1.format(formatter);
		...
		employees.forEach(employee -> {
			ArrayList<LeaveAllEmployee> allEmployees = new ArrayList<>();
			LeaveAllEmployeeRoot leaveAllEmployeeRoot = new LeaveAllEmployeeRoot();
			leaveAllEmployeeRoot.setEmployeeId(employee.getEmployeeId());
			leaveAllEmployeeRoot.setName(employee.getName());
			allEmployeesRoot.add(leaveAllEmployeeRoot);
		});
	...
	public Map<Object, Object> findHolidayByDate(String date) {
		Map<Object, Object> maps = new HashMap<>();
		String stDate = date + " " + "00:00:00";
		String enDate = date + " " + "00:00:00";
		List<Holiday> holiday1 = holidayRepository.findByStartAndEndDate(stDate, enDate);
		maps.put("holiday", holiday1);

		maps.put("status", !holiday1.isEmpty() && holiday1.size() > 0);
		return maps;
	}
	...
	public List<Holiday> findHoliday() {
		...
		return holidayRepository.findByType("Restricted Holiday");
	}
