...
public class AttendanceService implements AttendancesService {
	...
	public Map<String, Object> signInDecider(HttpServletRequest httpServletRequest) {
		...
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm:ss");
		String formatDateTime = now.format(formatter);
		String formatDateTime1=nextDate.format(formatter);
		String[] date = formatDateTime.split(" ");
		String[] date2 = formatDateTime1.split(" ");
		String dateTime = date[0] + " " + "00:00:00";
		String dateTime1 = date2[0] + " " + "00:00:00";
		System.out.println(dateTime1);
		System.out.println(dateTime);
		List<Attendance> list = attendanceRepo
				.findByEmployeeIdAndSignOutTimeIsNull(jwtTokenUtil.getToken(httpServletRequest), dateTime, dateTime1);
		response.put("signinStatus", !list.isEmpty() && list.size() > 0);
		DateTimeFormatter formatter1 = DateTimeFormatter.ofPattern("MM-dd-yyyy HH:mm:ss");
		if(!list.isEmpty())
		{
			list.forEach(e->{
				signInTime=e.getSignInTime();
				signIn=LocalDateTime.parse(signInTime,formatter);
			});
		}
